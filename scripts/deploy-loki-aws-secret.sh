...
create_s3_bucket(){
  if aws configure get region; then
      echo "bucket: Create s3 bucket $LOKI_BUCKET_NAME if not exist"
  else
      echo "bucket: Warning: Can not connect to aws, Insure bucket ready before deploy lokistack"
      return 0
  fi
  ...
  if [[ $? == 0 ]]  ;then
     echo "bucket: use existing bucket"
  else
