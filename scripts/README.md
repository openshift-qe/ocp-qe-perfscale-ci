...
Queries are sourced from the `netobserv-ipfix-metrics.yaml` file within the `scripts/` directory by default, but this can be overriden with the `--yaml-file` flag to run other queries from within other files such as `netobserv-ebpf-metrics.yaml`.
