...
template:
  ...
  spec:
    ...
    - name: "{{ .Chart.Name }}"
      ...
      command: ["sh", "-c", "cp /helm/config/helmcustomscan.policy /opt/rapidast/scanners/zap/policies/helm-custom-scan.policy && rapidast.py --config /helm/config/rapidastconfig.yaml --no-cleanup --log-level debug"]
