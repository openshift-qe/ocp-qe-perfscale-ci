        ...
        script{
            if (params.WORKER_COUNT.toInteger() > 0 ) {
                RETURNSTATUS = sh(returnStatus: true, script: '''
                    ...
                    SECONDS=0
                    current_worker_count=$(oc get nodes --no-headers -l node-role.kubernetes.io/worker | wc -l | xargs)
                    ...
                    if [[ $current_worker_count -ne $WORKER_COUNT ]]; then
                        ...
                        if [[ $current_worker_count -gt $WORKER_COUNT ]]; then
                            num_to_decrease=$(($current_worker_count-$WORKER_COUNT))
                            echo "num to decrease $num_to_decrease"
                            for machineset in $(oc get --no-headers machinesets -l machine.openshift.io/cluster-api-machine-role=worker -n openshift-machine-api | awk '{print $1}'); do
