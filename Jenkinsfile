...
pipeline {
 ...
 isn't stable<br>
  ...
  stages {
    stage('Configure given Flexy Cluster'){
      ...
      steps{
        ...
        withCredentials([file(credentialsId: 'b73d6ed3-99ff-4e06-b2d8-64eaaf69d1db', variable: 'OCP_AWS'),
          ...
          cp $OCP_AWS $WORKSPACE/ocp_aws.json
          cp $OCP_GCP $WORKSPACE/ocp_gcp.json
          cp $OCP_AZURE $WORKSPACE/ocp_azure.json
          oc config view
          oc projects
          ls -ls ~/.kube/
          env
          ...
          if [[ $(oc whoami --show-console | grep azure -c) == 1 ]]; then
          ...
          if [[ $(oc whoami --show-console | grep gcp -c) == 1 ]]; then
          ...
          if [[ $(oc whoami --show-console | grep aws -c) == 1 ]]; then
          mkdir ~/.aws
